...
import { type ICustomerSession } from "~/interface/talon-one.interface";
...
class TalonOneIntegrationAdapter {
	...
	constructor() {
		...
		defaultClient.basePath = config.public.t1.basePath;
		const api_key_v1 = defaultClient.authentications["api_key_v1"];
		api_key_v1.apiKey = config.public.t1.apiKey;
		api_key_v1.apiKeyPrefix = config.public.t1.prefixApiKey;
