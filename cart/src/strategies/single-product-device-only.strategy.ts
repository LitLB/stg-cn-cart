...
import { validateBulkDeleteCartItemBody, validateDeleteCartItemBody, validateProductQuantity, validateProductReleaseDate, validateSelectCartItemBody, validateSkuStatus, validateUpdateCartItemBody } from "../schemas/cart-item.schema";
...
export class SingleProductDeviceOnlyCartStrategy extends BaseCartStrategy {
    ...
    public async addItem(cart: Cart, payload: any): Promise<any> {
        try {
            ...
            const inventories = await this.adapters.commercetoolsInventoryClient.getInventory(sku);
    ...
    public async updateItem(cart: Cart, body: any): Promise<any> {
        try {
            ...
            const existingLineItem = this.adapters.commercetoolsMeCartClient.findLineItem({ cart, productId, variantId: variant.id, productGroup, productType, addOnGroup, freeGiftGroup });
            if (!existingLineItem) {
                throw {
                    statusCode: HTTP_STATUSES.BAD_REQUEST,
            ...
            const validateResult = await this.adapters.talonOneEffectConverter.validate(cart, changes)
            ...
            let updatedCart = await this.adapters.commercetoolsMeCartClient.updateItemQuantityInCart({
                cart,
                productId,
                variantId: variant.id,
                productGroup,
    ...
    public async removeItem(cart: Cart, body: any): Promise<any> {
        try {
            ...
            let updatedCart = await this.adapters.commercetoolsMeCartClient.removeItemFromCart({
                cart,
                productId,
                variantId: variant.id,
                productType,
    ...
    public async bulkRemoveItems(cart: Cart, body: any): Promise<any> {
        try {
            ...
            let updatedCart = await this.adapters.commercetoolsMeCartClient.removeItemsFromCart(cart, lineItemKeys);
