...
export class OtpService {
    ...
    public async requestOtp(phoneNumber: string) {
        ...
        try {
            ...
            logModel.logSuccess({ mobileNumber: phoneNumber, referenceCode: refCode, dateTime: moment().format('YYYY-MM-DD HH:mm:ss') }, data )
    ...
    const logModel = LogModel.getInstance();
    logModel.start_date = moment().toISOString();
    const logStepModel = createLogModel(LOG_APPS.STORE_WEB, LOG_MSG.CT_CHECK_ACTIVE_OPERATOR, logModel);
    try {
        ...
        logService({ operator, journey }, journeyActive, logStepModel)
        logModel.logSuccess({ operator, journey }, isActive)

        return isActive
    } catch (e: any) {
        logger.error('Error validate journeyActive')
        logService({ operator, journey }, e, logStepModel)
