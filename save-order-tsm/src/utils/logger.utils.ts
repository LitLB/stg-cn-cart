...
export class LogModel {
    ...
    constructor(req: LogModelRequest) {
        ...
        this.txid = req.txid;
        this.step_txid = req.step_txid;
        this.workflow_id = req.workflow_id;
        this.order_number = req.order_number;
    }
...
export function logService(req: any, res: any, l: LogModel) {
    try {
        ...
        const orderNumber = _.get(res, 'orderNumber', null) || _.get(res, 'response.orderNumber', null);
        const respCode = status || statusCode;
        if (resCode == '0' || respCode == '200' || respCode == '201' || respCode == '204' || orderNumber) {
...
export function createLogModel(app: string, msg: string = "", instLogModel?: LogModel): LogModel {
    ...
    if (instLogModel) {
        logRequest = {
            ...
            order_number: instLogModel.order_number,
